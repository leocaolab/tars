...
**Producer never reads back; the two halves are fed independently.** The producing pipeline feeds the read-able store and the write-only egress through two separate middleware — `TelemetryMiddleware` fires the redacted `tracing` signals (→ melt's `MetricsBridge` → M/L/T egress) and `EventEmitterMiddleware` writes the durable `PipelineEvent` (→ the `event/` store) — one per directional half. The pipeline NEVER reads the store back, and the egress is never gated on, nor reads from, the durable write, so ops stay visible even when the store lags.
