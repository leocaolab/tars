...
- Still pending for M5 (Doc 14 §11): `EnsemblePolicy` (fan-out/merge — a `RoutingService` dispatch restructure, not a policy plug-in); all metrics from Doc 08 §5, OTel SDK + OTLP exporter, cardinality validator, `SecretField<T>` generic wrapper (today `SecretString` covers the only consumer), trace head + tail sampling, `AdaptiveSampler`.
- **Trigger**: Latency + Cost policies landed. EnsemblePolicy + the OTel stack wait for demand / M5 proper.
