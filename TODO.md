...
- Still pending for M5 (Doc 14 §11): all metrics from Doc 08 §5, OTel SDK + OTLP exporter, cardinality validator, `SecretField<T>` generic wrapper (today `SecretString` covers the only consumer), trace head + tail sampling, `AdaptiveSampler`. (These are the `tars-melt` observability stack, not routing — routing's B-8 dependency is fully satisfied.)
