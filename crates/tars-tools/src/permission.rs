...
//! cannot reach for `tars_agent::Permissions`. The dispatch gate
...
/// What the policy decides for a tool invocation. Mirrors `tars_agent::Decision`
/// but lives here so the tool layer doesn't depend on `tars-agent`.
...
/// its `Permissions` without this crate knowing about `tars-agent`.
