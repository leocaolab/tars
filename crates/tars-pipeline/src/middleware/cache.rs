...
mod tests {
    ...
    /// Pin the production reader against the EXACT JSON shape any
    /// in-process writer of `cache.policy` will produce. The bespoke
    /// flat-JSON adapter went away with `ARC-L5-B-5` (no on-disk
    /// legacy payload existed, so the back-compat surface was code
    /// without a producer); now there is one wire shape — derived
    /// `Serialize` over the typed enum per layer.
    #[test]
    fn read_policy_decodes_canonical_wire_shapes() {
        let ctx = RequestContext::test_default();

        // Shape 1 — default-ish: l1 + l2 on, l3 off, no overrides.
        let default_wire = serde_json::json!({
            "l1": "default",
            "l2": "default",
            "l3": "disabled",
        });
        ctx.attributes
            .write()
            .unwrap()
            .insert(POLICY_ATTR.to_string(), default_wire);
        let p = read_policy(&ctx);
        ...
        let override_wire = serde_json::json!({
            "l1": {"override": {"ttl": {"secs": 300, "nanos": 0}}},
            "l2": "disabled",
            "l3": "disabled",
        });
        ctx.attributes
            .write()
            .unwrap()
            .insert(POLICY_ATTR.to_string(), override_wire);
        let p = read_policy(&ctx);
        assert_eq!(
            p.l1_ttl_effective(),
            Some(std::time::Duration::from_secs(300)),
            "override TTL must round-trip to Override variant"
        );
    }
}
