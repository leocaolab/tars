...
- **OTLP trace exporter shipped 2026-06-04 (opt-in)**: `tars-melt` `otlp` feature (off by default — heavy tonic/grpc stack) adds the OpenTelemetry SDK + `tracing-opentelemetry` bridge + OTLP/gRPC span exporter. Enable with `cargo build --features otlp` (or tars-cli's forwarding `otlp` feature) + `OTEL_EXPORTER_OTLP_ENDPOINT=...`; spans the whole stack already emits ship to any OTLP collector (Jaeger / Tempo / Datadog). `TelemetryGuard` flushes on drop. Bridge verified with an in-memory exporter test (no collector needed); smoke-tested through `tars run`. CI checks the feature explicitly (`cargo clippy/test -p tars-melt --features otlp`).
- Still pending for M5 (Doc 14 §11): metrics (Prometheus exporter — the OTel work so far is traces only), cardinality validator, `SecretField<T>` generic wrapper (today `SecretString` covers the only consumer), trace head + tail sampling, `AdaptiveSampler`. (These are the rest of the `tars-melt` observability stack, not routing.)
