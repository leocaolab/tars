...
| **没有隐藏的重试循环** | Reject 以 `ProviderError::ValidationFailed`（恒为 `ErrorClass::Permanent`）短路回 caller，中间件绝不重新调用模型。重新询问一个被拒绝的回答是 caller 的决定（catch 后再调一次 service），而不是一层 |
...
在 `LlmService::default_chain` 组装的链里，ValidationMiddleware 位于
**Cache 外、Retry 外**——见 [Doc 02](./02-middleware-pipeline.md)：
...
```
   EventEmitter? → Telemetry → ValidationMiddleware → Cache? → Retry → Provider
                  ...
                  └─────────────────────────────────┘
                       ...
                       │              ↳ ErrorClass::Permanent —— 直接短路,永不重试
...
- `Reject` 是一个**永久**判定：`ProviderError::ValidationFailed` 被分类为
  `ErrorClass::Permanent`，因此直接短路回 caller，没有任何层会重试它。
  （早期设计——"W1"——曾让 `Reject { retriable: true }` 冒泡到外层 RetryMiddleware，
  使被拒绝的回答重新调用模型。**该路径在 W4 被砍掉**；`ValidationOutcome::Reject`
  只携带一个 `reason`。不要再在别处重新记录它。）
- 位于 Cache 外意味着 validator 也能看到**缓存命中**，而不只是新鲜的 provider 响应
  ——交给 caller 的每一个回答都经过了校验。
- 它必须留在 **Provider 外**：provider 产出 token 流，而 validator 是 post-stream
  概念（见下），所以它只能在存在完整 `Response` 的地方运行。

如果你想让被拒绝的回答被重新询问，这在今天是 **caller** 的决定：catch
`ValidationFailed` 然后再调一次 service（和 provider fallback 同形——组合,而非一层）。
...
- `validation_reason: {kind, message, detail}` — 类型化的原因；fix-stage 应基于
  `validation_reason["kind"]` 分支，而不是解析 message 字符串
- 被分类为 `ErrorClass::Permanent`，所以抛出的异常的 `is_retriable` 恒为 `False`，没有任何层会重新调用模型
...
四个 outcome 工厂函数：`tars.Pass()` / `tars.FilterText(text, dropped)` / `tars.Reject(reason)`（或 `tars.Reject.typed(kind, message, detail)`） / `tars.Annotate(metrics)`。
...
理由：Reject 本来就（永久地）让这次 call 失败，没必要继续累计指标。
...
ValidationMiddleware 在 `Response.telemetry.layers` 里加一个 "validation" 标签。Caller 看到 layer chain 是 `["telemetry", "validation", "cache_lookup", "retry", "provider"]`。
