...
use tars_melt::event::{
    EvaluationScored, PipelineEvent, PipelineEventLog, PipelineEventQuery, SqlitePipelineEventLog,
...
fn open_store(dir: &str) -> PyResult<std::sync::Arc<dyn PipelineEventLog>> {
    ...
    // `open` is async (one SqlitePool per file); this helper is sync, so drive
    // it on the shared runtime.
    let store = crate::tokio_runtime()?
        .block_on(async {
            SqlitePipelineEventLog::open(SqlitePipelineEventLogConfig::new(path)).await
        })
