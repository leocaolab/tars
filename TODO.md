...
| M5 CLI/MELT      | ✅ shipped                    | tars-cli (init/probe/bench/plan/run/run-task/trajectory) + 3 dynamic routing strategies (`LatencyPolicy`/`CostPolicy`/`EnsembleService` via `tars.Pipeline.routed` + `tars run --route-by`) + the `tars-melt` OTel observability stack (opt-in `otlp` feature: OTLP trace + metric export, cardinality guard, head sampling). Post-M5 follow-ons (not blockers): `tars chat` REPL (B-5), tail/adaptive trace sampling (scale-gated), Prometheus *pull* exporter (OTLP push covers the need today). |
| M6 Multi-tenant + Server | 🟡 personal-mode server shipped | **`tars-server` shipped 2026-06-05** — personal-mode HTTP/REST shell over the pipeline: `GET /healthz`, `GET /v1/providers`, `POST /v1/complete` (sync JSON), `POST /v1/complete/stream` (SSE). Binds loopback by default; **no auth / no multi-tenant** (explicit subset). Still not started (the actual M6): `tars-security` (Auth/IAM/Secret resolution), per-tenant isolation + lifecycle, the Auth/IAM/Budget middleware onion layers, gRPC, Postgres/Redis stores. The server shell is ready to grow those in. |
| M7 Web frontends | 🟡 in progress (`tars-desktop`) | **Pivoted to a Tauri desktop debug GUI** (`tars-desktop`, LM-Studio-style: provider/model picker + chat + per-call metrics/trace + params panel) over `tars-runtime::Session` + `tars-server::AppState`. Doc 22 (Codex-TUI port) superseded. **A terminal `tars-tui` was attempted (vendored Codex's ratatui view onto upstream — `5f2c8fb`) then removed**: wrong modality (user wants a graphical app, not a character terminal; the Codex *app* GUI is closed-source, so the look is reimplemented, not vendored). **Strategy (Doc 22 §0): open-source + free** (more open than LangChain's paywalled LangGraph Studio/LangSmith — an adoption + differentiation wedge); **monorepo + CI-isolated now** (backend core `tars-desktop` in CI; Tauri shell `tars-desktop-app` excluded like `tars-py`), split to a same-org repo post-1.0; monetize the *hosted/team* layer later, never the local debugger. |
...
| M9 Output Validation + Eval | 🟢 W1+W2+W4+v3+v2 + W3 enabler + eval helper shipped | W3 enabler (Doc 17 Phase 1 — pipeline event store + body store + EventEmitter + `tars events` CLI + cohort tags) shipped 2026-05-08. **Re-scoped W3 main shipped 2026-06-04**: `tars.eval` Python helper (`read_calls` + `write_score`) over the event store — evaluator scripts read finished calls, compute a metric, append `EvaluationScored` events. Replaces the original "Rust evaluator runner + trait + sampler + subscription" (overengineered for batch-mode consumers). Also shipped this cycle: B-20.v2 (typed `ValidationReason`) + its event-log persistence + `tars events reasons` cohort view. Online monitoring (recall-drop alerts, context-saturation correlation) belongs in a separate Doc 19 / B-21 OTel exporter, not the evaluator path. |
...
- PyO3 wheel **shipped** — Stage 1+2+3 (Pipeline / Provider / Session / response_schema / `~/.tars/config.toml` / `tars init`). See CHANGELOG M8. Remaining items:
  ...
  - **B-6c. PyO3 `Pipeline.builder()` — ✅ shipped 2026-06-04 (configure/select built-ins).** Fluent `PipelineBuilder` pyclass: `.validators([...])` / `.event_store(dir)` / `.retry(max_attempts=…, initial_backoff_ms=…, …)` / `.cache(False)` / `.build()`, plus `config_path=` / `config_str=` provider sources. The canonical onion **order stays fixed** (load-bearing — Validation outside Cache per W4); the builder configures + opts out of layers, not reorders. Exposes the retry config that `from_default` left hardcoded, and a cache toggle. Added `PipelineOpts.cache: bool` Rust-side. 12 builder pytests (construction-only + one live layer-trace) + a Rust `default_chain_omits_cache_when_disabled` test.
    - **Still deferred (the original B-6c trigger): arbitrary *custom* Python middleware.** Bridging async Python logic into the Rust `LlmService` future/stream (GIL across await, non-Send Python objects in the stream) is genuinely hard (~weeks) and no consumer needs it. The sync-request-hook subset (auth-refresh / gating, validator-style `with_gil`) is the tractable future path if a real consumer asks.
...
### B-20.v2. Typed `ValidationOutcome::Reject { reason: ValidationReason }` — ✅ shipped 2026-06-03
- **Shipped**: `ValidationReason` enum (`#[non_exhaustive]`: `JsonShape{parse_error}` / `NotEmpty{field}` / `MaxLength{field,length,max}` / `Custom{kind,message,detail}`) in `tars-types/validation.rs` with `.kind()` + `Display` (Display reproduces the W1 message strings, so log scrapers / error-message contracts are unaffected). `ProviderError::ValidationFailed` now carries `reason: ValidationReason`. Built-in validators emit the typed variants; Python user validators + the adapter crash-fallback go through `Custom`. **No `retriable` field re-added** — W4's "always Permanent" stands; the original v2 spec's `retriable` mention is stale. Python: `tars.Reject("msg")` → `Custom{kind="user"}` (back-compat; `.reason` alias kept), new `tars.Reject.typed(kind, message, detail=None)`; `TarsProviderError` gains `validator: str` + `validation_reason: {kind, message, detail}`. Verified: 3 Rust assertions (typed variant + `.kind()`), 3 Python construction tests, 2 live-provider integration tests asserting `validation_reason` round-trips — all 25 tars-py pytests + tars-pipeline/types/runtime suites green.
- **Follow-up shipped 2026-06-03 (reject reason in the event log)**: `ValidationReason` now derives serde (externally tagged); `LlmCallFinished.validation_reason: Option<ValidationReason>` added — symmetric with `validation_summary` (success-path outcomes) since a Reject short-circuits before a Response and never lands in the summary. Wire-compatible via `#[serde(default, skip_serializing_if)]` (old blobs → `None`, non-reject events byte-identical). `EventEmitterMiddleware` (outside Validation in the onion) captures the typed reason on the `Err(ValidationFailed)` path. `CallResult::Error` stays the minimal `{kind}` rollup. Lets Doc 16 evaluators facet on *why* a validator rejected from the event store. 6 new serde/wire/integration tests; full workspace + 25 pytests green.
- **Original state (post-W1+W2, pre-ship)**: `Reject { reason: String }` — string-only. The Python side `TarsProviderError(kind="validation_failed", is_retriable=bool)` only stuffed the reason string into the message. Callers couldn't programmatically match against the failure reason.
...
- Mini version shipped — see CHANGELOG. **Per-provider latency slice shipped + LANDED 2026-06-04**: `LatencyStatsRegistry` (bounded rolling per-provider window + p50/p95/mean) + `LatencyPolicy` (reorders candidates fastest-first) + `RoutingService::with_latency_stats` (observes each successful dispatch) in tars-pipeline, **wired into a real consumer**: `Pipeline.chain_over` (extracted from `default_chain`) lets a `RoutingService` be the onion's inner, and tars-py `Pipeline.routed(provider_ids, policy="latency"|"static", latency_metric=, …)` exposes it with a `Pipeline.latency_stats()` readout. Verified with a **live e2e** (real completions feed real per-provider latency, readable from Python) plus the Rust mock-provider + policy unit tests. No longer speculative — it's reachable and exercised end to end.
- **`CostPolicy` shipped + landed 2026-06-04**: reorders candidates cheapest-first by *estimated* request cost from each provider's static `Pricing` (no runtime stats needed — pricing is config; a free local model wins, an unpriced provider sorts last). Exposed via `Pipeline.routed(policy="cost")`. Turned out *simpler* than LatencyPolicy, not harder — the static-pricing angle sidesteps the rolling cost-per-token problem; relative ordering is sound even with the char/4 token heuristic since it applies to all candidates equally.
- **`EnsembleService` shipped + landed 2026-06-04**: hedged fan-out — dispatch the request to all candidates in parallel, first response wins, rest cancelled. Implemented as a new `LlmService` (not a `RoutingPolicy` — it's a different dispatch shape, via `FuturesUnordered`), exposed via `Pipeline.routed(policy="ensemble")`. **All three routing strategies (latency / cost / ensemble) now done.** Output-*merging* ensembles (best-of-N / vote / judge) deliberately NOT built — they need a domain-specific merge over materialized responses, which has no universal library answer; "first response wins" is the generic streaming-safe variant.
- **CLI reach (2026-06-04)**: all three strategies are also reachable from `tars run --tier <T> --route-by <fallback|latency|cost|ensemble>` (second real consumer beyond tars-py). So routing is landed across both surfaces — Python `Pipeline.routed()` and the CLI.
- **OTLP trace + metric export shipped 2026-06-04 (opt-in)**: `tars-melt` `otlp` feature (off by default — heavy tonic/grpc stack) adds the OpenTelemetry SDK + `tracing-opentelemetry` trace bridge + OTLP/gRPC exporters. **Traces**: spans the whole stack already emits → any OTLP collector (Jaeger / Tempo / Datadog). **Metrics**: a `MetricsBridge` tracing layer turns the pipeline's existing `llm.call.{finished,failed}` events into OTel instruments (`tars.llm.calls` / `tars.llm.latency_ms` / `tars.llm.tokens`, attributed by `model` + `outcome`) — no instrumentation added to tars-pipeline, the events are the source. Enable with `cargo build --features otlp` (or tars-cli's forwarding feature) + `OTEL_EXPORTER_OTLP_ENDPOINT`. `TelemetryGuard` flushes both providers on drop. Verified with in-memory span + metric exporter tests (no collector); smoke-tested through `tars run`. CI checks the feature.
- **Cardinality guard + head sampling shipped 2026-06-05**, closing M5's MELT stack. `CardinalityGuard` caps each metric attribute key (e.g. `model`) at 100 distinct values, collapsing overflow into `__over_cardinality__` + a once-per-key warning — a runaway label can't melt the metrics backend. Head sampling via `OTEL_TRACES_SAMPLER_ARG` (parent-based traceidratio; `1.0`/AlwaysOn default) on the trace provider.
- Genuinely still deferred (no trigger): **tail + adaptive sampling** (`AdaptiveSampler`) — head sampling covers the v1 need; tail sampling is largely a collector-side concern. `SecretField<T>` — still YAGNI (`SecretString` covers the only consumer). Prometheus *pull* exporter — OTLP push covers the need; add only if someone wants scrape.
- **Trigger**: routing's metrics need (Latency/Cost/Ensemble) fully met. The remaining OTel/melt stack waits for M5 proper / a real export consumer.
