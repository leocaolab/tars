...
> 范围：在 LLM 返回 Response 之后、还给 caller 之前，按一组可注册的 `OutputValidator` 检查 / 改写 / 拒绝输出，把"校验输出契约"这件事从每个 consumer（ARC / agouflow / future scripts）各自实现 → 抽到 tars-pipeline 内置中间件。
>
> 上游：Doc 02 Middleware Pipeline（本文档新增的 ValidationMiddleware 是其中一层）；Doc 01 LLM Provider（`Response` / `ChatRequest` 类型）。
>
> 下游：所有需要"输出符合契约"的 consumer。第一个 user 是 ARC 的 critic agent，把目前内联在 `app/core/critic_agent.py` 的 `_known_rule_ids` 白名单逻辑迁过来。
>
...
ARC 那边 `except tars.TarsProviderError as e: if e.kind == "validation_failed": ...` 一行处理。
...
为什么这条作为 v1 第一个 built-in：ARC 的 critic 输出本来就是 strict JSON，schema 已经定好，跨 consumer 复用度最高。
...
`DemoteToAdHoc` 模式：把所有不在白名单的 rule_id 改写成 `"ad-hoc"`，在 evidence 字段里记录原值（跟 ARC 当前 inline 实现一致）。**Filter outcome**——返回改写后的 Response。
...
### 5.5 `EvidenceTagValidator`（ARC-specific，作为 example，不进 built-in）
...
**这条不进 built-in**——ARC-specific evidence schema。但 trait 形态完全一致，consumer 自己注册。Doc 给出实现示例方便复用。
...
- 测试：Validator 单元测试 + ARC 集成测试一起验
- 复用：agouflow / future tools 也想做这个时，import 同 validator 即可
...
| **W1.6** | 单元测试 + 集成测试 + ARC 迁移示例 + CHANGELOG | 0.5 天 |
| **总计** | | **~4.5 天** |

### 11.2 落地后立刻动作

1. ARC 把 `_known_rule_ids` 迁出来（B-15 TODO 现行已注，参见 ARC 仓库 commit `1fe6cbc`）
2. ARC 把 `MaxLengthValidator` 接上（防 prompt injection 输出爆）
3. 写一个 `EvidenceTagValidator`（ARC 专用）作为 plugin example，进 ARC 仓库 `arc/validators.py`
