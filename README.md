...
For a guided tour by role (architect / SDK author / SRE / security), see [docs/00-overview.md](./docs/00-overview.md).
...
`Session.history_version: u64` increments on visible mutations (successful send, reset). Does **not** increment on rollback (truncating back to pre-send is observably unchanged) or during in-flight tool loops. Borrowed from codex-rs's `ContextManager`. Useful for cache invalidation and `(session_id, history_version)` log correlation. `fork()` preserves the parent value so caches recognize shared prefixes.
...
Borrowed scaffolds and speculative abstractions are tracked in [TODO.md §O-1..O-10](./TODO.md). Each entry has:
...
The 2026-05-03 ARC self-review surfaced ~330 issues across three rounds. Critical + error tier was triaged and shipped (commits `9683ce8` / `67de40d` / `cf1605e` / `af2d8f1`). Non-critical residue lives in [TODO.md §A-1..A-6](./TODO.md) with revisit triggers, not "we'll fix it eventually."
...
Borrowed patterns acknowledged in CHANGELOG with attribution:
- **codex-rs**: `HttpTransport` trait shape, `ContextManager.history_version`, `SafetyCheck` 3-state pattern (we adopted 2-state with explicit reasoning — see B-31)
- **opencode**: `.txt` prompts pattern, `ToolResult.title`, `Retry-After` header parsing
- **claude-code**: CLI subprocess long-lived stream-json pattern (still inspiration, not yet shipped — see [TODO.md B-1](./TODO.md))

Borrowed scaffolds carry trigger-or-delete contracts (see [Engineering practice](#engineering-practice) above).
