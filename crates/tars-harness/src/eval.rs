...
use tars_types::{
    ChatRequest, ChatResponse, ChatResponseBuilder, ProviderId, RequestContext, Usage,
};

/// Config for [`run_eval`] — a corpus replay through a pipeline. Plain
/// value struct (no clap): the CLI resolves config → registry → provider
/// and hands the engine everything it needs.
pub struct EvalRunConfig {
    /// Provider registry the run's pipeline (and any judge) is drawn from.
    pub registry: Arc<ProviderRegistry>,
    /// Provider id the run replays through (already resolved by the caller).
    pub provider_id: ProviderId,
    /// Model hint. `None` → the provider's default model.
    pub model: Option<String>,
    /// Corpus directory. Each subdirectory is one case.
    pub corpus: PathBuf,
    /// Output directory. `None` → `./benchmarks/runs/eval/<timestamp>/`.
    pub output: Option<PathBuf>,
    /// Per-case max output token bound.
    pub max_output_tokens: Option<u32>,
    /// Built-in invariant `--check` specs.
    pub checks: Vec<String>,
    /// Run each case through a read-only tool-using agent loop.
    pub agent: bool,
    /// In agent mode, which read-only tools to expose (empty → all four).
    pub tools: Vec<String>,
    /// In agent mode, cap the tool-loop iterations per case.
    pub agent_max_iterations: Option<u32>,
    /// Judge provider for `trajectory-match:args-judge` (anti-incest vs `provider_id`).
    pub judge_provider: Option<String>,
    /// Judge model hint for `judge_provider`.
    pub judge_model: Option<String>,
}

/// Config for [`run_judge`] — an LLM judge over an eval run's outputs.
pub struct EvalJudgeConfig {
    /// Provider registry the judge is drawn from.
    pub registry: Arc<ProviderRegistry>,
    /// Eval-run directory (contains manifest.json + per-case output.txt).
    pub run: PathBuf,
    /// Provider id to use as the judge (anti-incest vs the run's provider).
    pub judge: String,
    /// Judge model hint. `None` → the provider's default model.
    pub judge_model: Option<String>,
}

/// Config for [`run_diff`] — a behavioral diff of two eval runs.
pub struct EvalDiffConfig {
    ...
    pub json: bool,
    /// Add the tool-trajectory section (head-to-head divergence + McNemar).
    pub trajectory: bool,
    /// Similarity mode for the head-to-head divergence: exact | ordered | set.
    pub trajectory_mode: String,
}

/// Config for [`run_bless`] — the approval loop over an eval run's outputs.
pub struct EvalBlessConfig {
    /// Eval-run directory (contains manifest.json + per-case output.txt).
    pub run: PathBuf,
    /// JSONPath-subset fields to bless. Non-empty → record mode; empty → check.
    pub select: Vec<String>,
    /// Write the bless directly (accept). Without it, record mode stages a
    /// `*.bless.json.new` for review.
    pub accept: bool,
}
...
pub fn run_bless(cfg: EvalBlessConfig) -> Result<()> {
    ...
    for case in &manifest.cases {
        let case_dir = cfg.run.join(&case.case_id);
...
pub async fn run_judge(cfg: EvalJudgeConfig) -> Result<()> {
    use crate::{JudgeItem, LlmJudge, ensure_anti_incest, run_judge_pass};
    ...
    // Build the judge pipeline from the provided registry.
    let judge_pid = ProviderId::new(&cfg.judge);
    let judge_provider = cfg
        .registry
        .get(&judge_pid)
        .ok_or_else(|| anyhow::anyhow!("judge provider `{}` not in config", cfg.judge))?;
    let judge_pipeline = LlmService::default_chain(
        judge_provider,
        cfg.judge_model.clone().unwrap_or_default(),
    ...
    let judge = LlmJudge::new(
        ...
        format!(
            "{}:{}",
            cfg.judge,
            cfg.judge_model.as_deref().unwrap_or("default")
        ),
    );
    ...
    if items.is_empty() {
        anyhow::bail!("no successful cases in {} to judge", cfg.run.display());
    }
    ...
    let report_path = cfg.run.join("judge_report.json");
...
pub fn run_migrate_checks(dir: &Path) -> Result<()> {
    let touched = migrate_legacy_check_results_in_dir(dir)?;
    if touched.is_empty() {
        println!(
...
struct TrajDiff {
    ...
    mcnemar: Vec<(String, crate::McNemarResult)>,
}
...
pub fn run_diff(cfg: EvalDiffConfig) -> Result<()> {
    let a = load_manifest(&cfg.baseline)?;
    let b = load_manifest(&cfg.candidate)?;

    // Tool-trajectory section is opt-in (--trajectory). Parse the mode early so
    // a bad value fails before any output.
    let traj_diff = if cfg.trajectory {
        let mode = MatchMode::parse(&cfg.trajectory_mode).ok_or_else(|| {
            anyhow::anyhow!(
    ...
    if cfg.json {
        ...
        let mut out = serde_json::json!({
            "baseline": cfg.baseline.display().to_string(),
            "candidate": cfg.candidate.display().to_string(),
            "operational": {
    ...
    // Human table.
    println!("eval diff");
    println!("  baseline:  {}", cfg.baseline.display());
    println!("  candidate: {}", cfg.candidate.display());
    println!();
    println!("operational:");
    println!(
        "  {:<14} {:>10} → {:>10}   {}",
    ...
    if let (Some(ja), Some(jb)) = (
        load_judge_report(&cfg.baseline),
        load_judge_report(&cfg.candidate),
    ) {
        use crate::{JudgeVerdict, mcnemar};
        println!();
        ...
        let to_map = |r: &crate::JudgeReport| {
            r.verdicts
                .iter()
                .map(|v| {
                    (
...
fn load_judge_report(dir: &Path) -> Option<crate::JudgeReport> {
...
pub async fn run_eval(cfg: EvalRunConfig) -> Result<()> {
    ...
    if cfg.agent {
        anyhow::bail!(
            "--agent eval is retired: it ran on the deleted WorkerAgent model; \
             port to the new Agent/board model is a separate task"
        );
    }
    if !cfg.tools.is_empty() || cfg.agent_max_iterations.is_some() {
    ...
    let arg_judge = if traj_specs.iter().any(|t| t.judge) {
        ...
        let jid = format!("{}:{}", jp, cfg.judge_model.as_deref().unwrap_or("default"));
        Some(ArgEquivalenceJudge::new(jpipeline, jid))
    } else {
        if cfg.judge_provider.is_some() || cfg.judge_model.is_some() {
            anyhow::bail!(
...
///
/// **UNWIRED.** Nothing outside this file's tests calls it, so `--agent` mode does not
/// currently get the jail this function exists to install. It is written and tested;
/// it was never installed.
///
/// `#[allow(dead_code)]` rather than `#[cfg(test)]`: marking it test-only would be a
/// lie about what it is for, and would freeze the gap by making the compiler stop
/// mentioning it. The warning was the only thing pointing at a missing security
/// control, so the note replaces it rather than the silence.
#[allow(dead_code)]
