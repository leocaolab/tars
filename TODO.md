...
| M5 CLI/MELT      | 🟡 partial                   | tars-cli shipped. **All 3 dynamic routing strategies shipped + landed 2026-06-04** (B-8): `LatencyPolicy` (observed latency) + `CostPolicy` (static pricing) + `EnsembleService` (hedged fan-out), all via `tars.Pipeline.routed(policy=…)`. Still pending: the OTel/`tars-melt` observability stack (Prometheus exporter, cardinality validator, sampling) — not a routing blocker. |
...
- Still pending for M5 (Doc 14 §11): **cardinality validator** — *trigger now fired* (metrics exist), so it's the natural next melt item: validate that metric attribute sets (`model` etc.) can't explode Prometheus cardinality. **Trace sampling** (head/tail, `AdaptiveSampler`) — deferred on purpose: AlwaysOn is correct until trace volume becomes a cost problem (scale trigger). `SecretField<T>` — still YAGNI (`SecretString` covers the only consumer). Prometheus *pull* exporter (vs the OTLP push shipped here) — only if someone wants scrape instead of push.
