...
from .types import Blackboard, Patch, IEvaluator, IActor, DirectiveExtractor
...
class AgentEngine:
    ...
    def push_input(self, board: Blackboard, user_input: str) -> str:
        """处理一轮用户输入，返回 Actor 的回复。

        完整流程：追加输入 → Evaluator 评估 → 合并补丁 → 提取指令 → Actor 回复。
        """
